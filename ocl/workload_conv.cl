...
__kernel void workload1_convolution(__global float *inputs, 	__global float *weights, 	
								...
								)
{
	...
	__local float localFilter[LOCAL_DEPTH_1][9];
	...
	__private float ans[LOCAL_DEPTH_1] = {0.0}; 
	...
	for (c=0; c<C; c++) {
		...
		for (idx=local_idx; idx<LOCAL_DEPTH_1*9; idx+=13*13) {
			int chunk_num = idx/9, pos = idx%9;
			localFilter[chunk_num][pos] = weights[(global_z*LOCAL_DEPTH_1 + chunk_num)*C*9 + c*9 + pos];
		}
		barrier(CLK_LOCAL_MEM_FENCE);

		for (d=0; d<LOCAL_DEPTH_1; d++) {
			for (h=0; h<3; h++) {
				for (w=0; w<3; w++) {
...
__kernel void workload2_convolution(__global float *inputs, 	__global float *weights, 	
								__global float *outputs,
								const int K, 				const int C, 		
								const int W, 				const int H,
								const int k_size,  			const int out_w,
...
__kernel void workload4_convolution(__global float *inputs, 	__global float *weights, 	
								__global float *outputs,
								const int K, 				const int C, 		
								const int W, 				const int H,
								const int k_size,  			const int out_w,
...
__kernel void workload8_convolution(__global float *inputs, 	__global float *weights, 	
								__global float *outputs,
								const int K, 				const int C, 		
								const int W, 				const int H,
								const int k_size,  			const int out_w,
...
__kernel void workload16_convolution(__global float *inputs, 	__global float *weights, 	
								__global float *outputs,
								const int K, 				const int C, 		
								const int W, 				const int H,
								const int k_size,  			const int out_w,
...
__kernel void workload32_convolution(__global float *inputs, 	__global float *weights, 	
								__global float *outputs,
								const int K, 				const int C, 		
								const int W, 				const int H,
								const int k_size,  			const int out_w,
